...
USER_COLUMN_VALUES_TO_KEYWORD = {
    'connection_limit': {'UNLIMITED': None}
}
...
def get_user_attributes(user_attributes):
    ...
    user_attributes = {USER_COLUMN_NAME_TO_KEYWORD[column_name]: value
                        for column_name, value in user_attributes.items()
                        if column_name in USER_COLUMN_NAME_TO_KEYWORD}
    user_attributes = {column_name: USER_COLUMN_VALUES_TO_KEYWORD.get(column_name, {}).get(value, value)
                        for column_name, value in user_attributes.items()}
...
def get_all_user_attributes(cursor):
    ...
    all_user_attributes = {user: get_user_attributes(pg_attributes)
                            for user, pg_attributes
                            in all_pg_user_attributes.items()}
