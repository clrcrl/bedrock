...
def get_alter_user_queries(user, spec_attributes, current_attributes):
    ...
    """
    sql_to_run = []
    desired_attributes = get_desired_attributes(spec_attributes, DEFAULT_ATTRIBUTES)
    for attribute, spec_value in desired_attributes.items():
        current_value = current_attributes.get(attribute)
        if attribute == "password":
            sql_to_run.append("-- " + Q_ALTER_PASSWORD.format(user, "******"))
        else:
...
def get_create_user_queries(user, spec_attributes):
    ...
    sql_to_run.extend(get_alter_user_queries(user, spec_attributes, DEFAULT_ATTRIBUTES))
    return sql_to_run
...
def get_users_sql(spec, cursor):
    ...
    for user, spec in spec_users.items():
        ...
        if user not in current_users:
            ...
            sql_to_run.extend(
                get_alter_user_queries(user, spec_attributes, current_attributes)
            )
            all_password_sql_to_run.extend(get_password_sql(user, spec_attributes))
